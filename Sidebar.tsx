  ...
  return (
    <aside className="w-72 sm:w-80 h-full bg-slate-900 border-r border-slate-800 flex flex-col z-10 shrink-0 text-slate-200">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar">
        ...
        {activeTab === 'bones' && (
          <>
            {/* 1-Click Automatic Character Rigging */}
            <div className="space-y-2 pb-1">
              <button
                id="btn_sidebar_auto_rig"
                onClick={() => studioStore.autoRig()}
                title="Automatically detect character silhouette, place bones, fit bone widths & compute weights"
                className="w-full py-2.5 px-3 bg-gradient-to-r from-sky-500 to-indigo-500 hover:from-sky-600 hover:to-indigo-600 text-white rounded-xl text-xs font-bold flex items-center justify-center gap-2 shadow-lg shadow-sky-500/25 transition"
              >
                <Wand2 className="w-4 h-4 text-amber-300" />
                <span>Auto-Rig Character</span>
              </button>

              <div className="bg-slate-800/40 border border-slate-700/50 rounded-xl p-2 space-y-1.5">
                <div className="flex items-center justify-between text-[11px] px-1 text-slate-400">
                  <span>Character Templates</span>
                  <span className="text-[10px] text-sky-400 font-medium">Auto-Detect / Preset</span>
                </div>
                <div className="grid grid-cols-4 gap-1">
                  <button
                    id="btn_preset_human"
                    onClick={() => studioStore.selectPreset('human')}
                    className="py-1.5 px-1 bg-slate-800/80 hover:bg-slate-700 text-slate-300 hover:text-white rounded-lg text-[10px] font-medium text-center border border-slate-700/60 transition"
                  >
                    Human
                  </button>
                  <button
                    id="btn_preset_biped"
                    onClick={() => studioStore.selectPreset('biped')}
                    className="py-1.5 px-1 bg-slate-800/80 hover:bg-slate-700 text-slate-300 hover:text-white rounded-lg text-[10px] font-medium text-center border border-slate-700/60 transition"
                  >
                    Biped
                  </button>
                  <button
                    id="btn_preset_quadruped"
                    onClick={() => studioStore.selectPreset('quadruped')}
                    className="py-1.5 px-1 bg-slate-800/80 hover:bg-slate-700 text-slate-300 hover:text-white rounded-lg text-[10px] font-medium text-center border border-slate-700/60 transition"
                  >
                    Beast
                  </button>
                  <button
                    id="btn_preset_fish"
                    onClick={() => studioStore.selectPreset('fish')}
                    className="py-1.5 px-1 bg-slate-800/80 hover:bg-slate-700 text-slate-300 hover:text-white rounded-lg text-[10px] font-medium text-center border border-slate-700/60 transition"
                  >
                    Fish
                  </button>
            ...
            <div className="grid grid-cols-3 gap-1.5 pt-1">
              <button
                ...
                className="px-2 py-2 bg-slate-800/70 hover:bg-slate-750 border border-slate-700 rounded-xl text-[11px] font-medium flex items-center justify-center gap-1 text-slate-300 hover:text-white transition"
              >
              ...
              <button
                id="btn_auto_widths"
                onClick={() => studioStore.autoBoneWidths()}
                title="Automatically calculate envelope widths from character geometry"
                className="px-2 py-2 bg-slate-800/70 hover:bg-slate-750 border border-slate-700 rounded-xl text-[11px] font-medium flex items-center justify-center gap-1 text-slate-300 hover:text-white transition"
              >
                ...
                <div className="p-2.5 bg-slate-900/80 rounded-xl border border-slate-800 space-y-2.5">
                  <div className="flex items-center justify-between text-[11px]">
                    <span className="font-semibold text-sky-400 flex items-center gap-1">
                      <span>Envelope Widths</span>
                    </span>
                    <div className="flex items-center gap-1.5">
                      <button
                        id="btn_auto_fit_bone_width"
                        onClick={() => studioStore.autoBoneWidths(selectedBone.id)}
                        title="Auto-calculate envelope widths for selected bone from character geometry"
                        className="px-1.5 py-0.5 bg-sky-500/20 hover:bg-sky-500/30 text-sky-300 border border-sky-500/30 rounded text-[10px] font-medium flex items-center gap-1 transition"
                      >
