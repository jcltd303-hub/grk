    ...
    <div className="w-screen h-screen flex flex-col bg-slate-950 text-slate-100 overflow-hidden select-none font-sans">
      ...
      <div className="h-9 bg-slate-900/90 border-b border-slate-800/80 flex items-center justify-between px-3 shrink-0 z-10 text-xs overflow-x-auto custom-scrollbar gap-2">
        <div className="flex items-center gap-1.5 shrink-0">
          ...
          <button
            id="btn_toolbar_auto_rig"
            onClick={() => studioStore.autoRig()}
            title="Automatically create bones, fit widths & bind skin weights"
            className="px-2.5 py-1 bg-gradient-to-r from-sky-500/20 to-indigo-500/20 hover:from-sky-500/30 hover:to-indigo-500/30 border border-sky-500/40 text-sky-200 rounded-lg font-semibold flex items-center gap-1 transition text-[11px] shadow-xs"
          >
          ...
          <button
            id="btn_toolbar_auto_widths"
            onClick={() => studioStore.autoBoneWidths()}
            title="Automatically set bone widths to enclose character geometry"
