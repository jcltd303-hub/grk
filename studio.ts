...
import {
  ...
  calculateAutomaticBoneWidths,
  calculateSingleBoneWidth,
} from '../lib/rig/auto-rig';
...
export interface StudioState {
  ...
  loadCustomImage: (dataUrl: string, presetType?: PresetType, autoRig?: boolean) => void;
  autoRig: (type?: PresetType) => void;
  autoBoneWidths: (boneId?: string) => void;
  calculateAutomaticBoneWidths: () => void;
...
class StudioStore {
  ...
  constructor() {
    this.state = {
      ...
      loadCustomImage: (url, type, autoRig) => this.loadCustomImage(url, type, autoRig),
      autoRig: (type) => this.autoRig(type),
      autoBoneWidths: (boneId) => this.autoBoneWidths(boneId),
      calculateAutomaticBoneWidths: () => this.autoBoneWidths(),
  ...
  public selectPreset(presetId: string) {
    ...
    if (preset) {
      this.loadCustomImage(preset.imageUrl, preset.type, true);
    } else {
      this.loadStarterRig();
    }
  }

  public loadCustomImage(dataUrl: string, presetType?: PresetType, autoRig: boolean = true) {
    this.loadArtworkImage(dataUrl, presetType, autoRig);
  }
   ...
   */
  private loadArtworkImage(imgSrc: string, presetType?: PresetType, autoRigOnLoad: boolean = true) {
    const img = new Image();
    img.crossOrigin = 'anonymous';
    img.src = imgSrc;
    ...
    img.onload = () => {
      ...
      let restSkeleton: Skeleton;

      if (autoRigOnLoad) {
        const rigged = autoRig(mesh, w, h, alphaMask, presetType);
        skeleton = rigged.skeleton;
        restSkeleton = rigged.restSkeleton;
      } else {
        skeleton = createEmptySkeleton(w, h);
        restSkeleton = createEmptySkeleton(w, h);
      }

      const pType = presetType || (skeleton.bones.length > 0 ? (skeleton.bones[0].name.toLowerCase().includes('pelvis') ? 'human' : 'human') : 'human');
      const clips = getDefaultAnimationClips(pType);
      ...
      this.setState({
        ...
        skeleton,
        restSkeleton,
        mode: autoRigOnLoad ? 'pose' : 'rig',
        tool: autoRigOnLoad ? 'select' : 'add_bone',
        selectedBoneId: skeleton.bones.length > 0 ? skeleton.bones[0].id : null,
        clips,
        activeClipId: activeClip ? activeClip.id : null,
        currentTime: 0,
        isPlaying: false,
  ...
  public autoRig(type?: PresetType) {
    const { image, mesh, alphaMask } = this.state;
    if (!mesh) return;

    this.saveHistory();
    const w = image?.naturalWidth || image?.width || mesh.width;
    const h = image?.naturalHeight || image?.height || mesh.height;

    const rigged = autoRig(mesh, w, h, alphaMask, type);
    const clips = getDefaultAnimationClips(type || 'human');
    ...
    this.setState({
      ...
      selectedBoneId: rigged.skeleton.bones.length > 0 ? rigged.skeleton.bones[0].id : null,
    });
  ...
  public autoBoneWidths(boneId?: string) {
    const { skeleton, restSkeleton, mesh, alphaMask, image } = this.state;
    ...
    if (boneId) {
      ...
      if (bone) {
        const { startWidth, endWidth } = calculateSingleBoneWidth(bone, skeleton, mesh, alphaMask, w, h);
        bone.startWidth = startWidth;
        bone.endWidth = endWidth;
        if (restSkeleton) {
          const rb = restSkeleton.bones.find((r) => r.id === bone.id);
          if (rb) {
            rb.startWidth = startWidth;
            rb.endWidth = endWidth;
          }
        }
      }
    } else {
      calculateAutomaticBoneWidths(skeleton, mesh, alphaMask, w, h);
      if (restSkeleton) {
        for (const b of skeleton.bones) {
  ...
  public moveBoneJoint(boneId: string, jointType: 'start' | 'end', newPos: Point2D) {
    ...
    if (mode === 'rig') {
      ...
      if (mesh) resetMeshToRest(mesh);
      if (mesh) {
        const { alphaMask, image } = this.state;
        const imgW = image?.naturalWidth || image?.width || mesh.width;
        const imgH = image?.naturalHeight || image?.height || mesh.height;
        const { startWidth, endWidth } = calculateSingleBoneWidth(bone, skeleton, mesh, alphaMask, imgW, imgH);
        bone.startWidth = startWidth;
        bone.endWidth = endWidth;
      }
      this.state.restSkeleton = cloneSkeleton(skeleton);
  ...
  public addBone(
    ...
    if (!skeleton || skeleton.bones.length === 0) {
      ...
      const autoW = calculateSingleBoneWidth(dummyBone, { bones: [dummyBone], rootId: dummyBone.id, rootPos: { ...rootStart }, restRootPos: { ...rootStart }, restBones: {} }, mesh, this.state.alphaMask, imgW, imgH);
      const startWidth = options?.startWidth ?? autoW.startWidth;
      const endWidth = options?.endWidth ?? autoW.endWidth;
    ...
    const inheritedStartWidth = branchFromStart
      ...
      : (parent.endWidth ?? 20);
    const dummyChild: Bone = {
      id: 'tmp_child',
      name: 'TempChild',
      parentId: parent.id,
      localAngle,
      length,
      startWidth: inheritedStartWidth,
      endWidth: 18,
      color: BONE_PALETTE[0],
      start: { ...boneStartPos },
      end: {
        x: boneStartPos.x + Math.cos(normalizeAngle(parent.worldAngle + localAngle)) * length,
        y: boneStartPos.y + Math.sin(normalizeAngle(parent.worldAngle + localAngle)) * length,
      },
      worldAngle: normalizeAngle(parent.worldAngle + localAngle),
    };
    const autoChildW = calculateSingleBoneWidth(dummyChild, skeleton, mesh, this.state.alphaMask, imgW, imgH);
    const startWidth = options?.startWidth ?? (branchFromStart ? inheritedStartWidth : (parent.endWidth || autoChildW.startWidth));
    const endWidth = options?.endWidth ?? autoChildW.endWidth;
  ...
  public recomputeWeights() {
    const { mesh, skeleton, alphaMask, image } = this.state;
    if (mesh && skeleton) {
      const w = image?.naturalWidth || image?.width || mesh.width;
      const h = image?.naturalHeight || image?.height || mesh.height;
      calculateAutomaticBoneWidths(skeleton, mesh, alphaMask, w, h);
